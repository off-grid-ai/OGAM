...
| T016 | 🔴 P2 | ❌ DEFERRED (no surface + fake knob) | GPU backend first load; 8s init-timeout → retry → 24/36 | load succeeds without a silent hang. **DEFERRED — the 8s timing has NO rendered surface (plan: drop the raw-timing sub-assertion); the only testable outcome (graceful GPU→CPU fallback) needs a "first GPU initLlama rejects/hangs" fake knob the llama boundary doesn't expose. Low value (the fallback path is exercised indirectly by T014's CPU sibling). Would need an additive `installNativeBoundary` GPU-init-failure knob** | DEV-B24 · timeout→24/36 | |
...
| T021 | 🔴 P2 | ❌ DEFERRED (harness gap + surface mismatch) | Load a vision gguf (gemma-4-E2B + mmproj) via select+send | estimate not mmproj-inflated. **DEFERRED — GenerationMeta GPU/CPU is the WRONG surface (Android nGpuLayers ignores the estimate → would duplicate T014); B3's real symptom is a FALSE memory-refusal card, but the harness can't seed `mmProjFileSize` to inflate the estimate. Narrow variant of the T024/T027/T028 estimator family. See `RESIDENCY_TEST_MISMATCHES.md`** | DEV-B3 · CPU-fallback | |
