...
const AudioBusyFace: React.FC<{ kind: 'loading' | 'transcribing' }> = ({
  ...
  kind === 'loading' ? (
    <LoadingState asSendButton={false} />
  ) : (
    <TranscribingState asSendButton={false} />
  );
...
const ChatButtonFace: React.FC<{
  ...
  if (kind === 'loading') return <LoadingState asSendButton />;
  if (kind === 'transcribing') return <TranscribingState asSendButton />;
  return (
...
export const VoiceRecordButton: React.FC<VoiceRecordButtonProps> = ({
  ...
  const callbacksRef = useRef<VoiceRecordCallbacks>({
    onStartRecording,
    onStopRecording,
    onCancelRecording,
    onInteractionModeChange,
  ...
  if (
    ...
    return (
      <View style={styles.container}>
        <AudioBusyFace kind={buttonState.kind} />
        {alert}
      </View>
    );
  }
      ...
      <View style={styles.container}>
        ...
        >
          ...
          >
            ...
            >
              {isRecording ? (
                <Icon name="square" size={24} color={colors.surface} />
              ) : (
                <ButtonIcon
                  asSendButton={false}
                  isRecording={false}
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View
        ...
        accessibilityLabel={
          tapLockedRef.current || isRecording
