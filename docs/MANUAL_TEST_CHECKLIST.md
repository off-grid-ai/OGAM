...
| T072 | 🔴 P1 | ✅ `enhancementReasoningPrompt.rendered.redflow` | Enhance + thinking ON → send "draw a cat"; model reasons when thinking is on | the prompt reaching the image generator is the clean rewrite, NOT the model's reasoning chain (RED: `enable_thinking=true` → "Thinking Process:…" becomes the image prompt — B30's slow/garbage symptom at the OUTCOME altitude, complements T071's request-param check). Emergent: the fake emits the reasoning dump ONLY when enable_thinking===true, so it's the app's own decision. Falsified: thinking off → clean rewrite → green | DEV-B30 · SLOW | |
