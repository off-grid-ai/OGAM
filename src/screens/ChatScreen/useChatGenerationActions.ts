...
async function generateWithCompactionRetry(
  ...
  const gen = (msgs: Message[], includeOptionalTools = true) =>
    includeOptionalTools && (enabledTools.length > 0 || extCount > 0)
  ...
  try {
    ...
    if (carriesImage && (enabledTools.length > 0 || extCount > 0)) {
      logger.log(
        '[GEN-SM] context recovery preserves image and omits optional tool schemas',
      );
    }
