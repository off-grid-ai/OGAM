...
| T062 | ✅ P1 | ~ FIXED (recordedTurnKind replay); green guard Phase 2 | Send "draw a dog" (routes to IMAGE ✓) → open action menu (long-press/3-dots) → tap **Regenerate/Resend** | resend STILL routes to IMAGE (re-runs ROUTE-SM classify) (RED: resend jumps to LLM text path, no classify → text answer). Falsify: fresh "draw a dog" → image | DEV-B33 · BROKEN | |
