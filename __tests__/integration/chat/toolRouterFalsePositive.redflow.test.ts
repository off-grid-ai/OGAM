 ...
 *
 * Drives the real Shared model selector — the only faked boundary is the model text
 * (the `generate` callback), i.e. what the router LLM would say. This is the router's real substring
 * parser: a name that appears in model prose must not become a false-positive selection.
 * `selected.length > 0` returns BEFORE the `'none'` check — so a decline that names the tool selects it.
 *
 * DOCUMENTED EXCEPTION to the UI-driven standard: this bug lives in a pure Shared parser,
...
describe('Q4 — tool router false-positive (red-flow)', () => {
  it('selects NO tool when the router declines but happens to name one', async () => {
    ...
    const selected = await selectRelevantToolsWithModel('hello there', TOOLS, generate);
  ...
  it('control: when the router names ONLY the tool it wants, that tool is selected', async () => {
    ...
    const selected = await selectRelevantToolsWithModel('what is the weather in Paris', TOOLS, generate);
