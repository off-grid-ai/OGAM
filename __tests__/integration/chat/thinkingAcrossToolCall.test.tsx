...
describe('thinking across a tool-call turn (guard)', () => {
  it('shows the streamed reasoning AND the final answer after a tool call', async () => {
    ...
    useChatStore
      .getState()
      .addMessage(conversationId, { role: 'user', content: 'what is 2+2' });
    ...
    const reasoningView = render(
      React.createElement(ChatMessage, {
        message: toolAssistant as Message,
        showGenerationDetails: false,
      }),
    );
    const answerView = render(
      React.createElement(ChatMessage, {
        message: finalAssistant as Message,
        showGenerationDetails: false,
      }),
    );

    // The reasoning is preserved ONCE on the assistant turn that led to the tool;
    // the separate final assistant owns only the post-tool answer.
    expect(
      reasoningView.queryByText(/Let me compute this with the calculator/),
    ).not.toBeNull();
    expect(answerView.queryByText(/The answer is 4\./)).not.toBeNull();
  });
});
