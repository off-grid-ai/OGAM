...
describe('D4 — iOS interrupted download leaves no failed entry (red-flow)', () => {
  it('surfaces an interrupted iOS download as a failed/retriable entry after app-kill', async () => {
    ...
    await AsyncStorage.setItem('@offgrid/model_downloads_v2', JSON.stringify([{
      manifest: {
        id: 'gemma-4b', modelId: 'gemma-4b', kind: 'text', revision: 'main',
        artifacts: [{id: 'primary', name: 'gemma-4b.gguf', role: 'primary', required: true, localName: 'gemma-4b.gguf'}],
      },
      phase: 'downloading',
