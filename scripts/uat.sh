...
# clean checkout is dirty by the time we get here. Neither is source we build from.
[ -z "$(git status --porcelain | grep -vE '(fastlane/README\.md|\.bundle/config)$' || true)" ] || error "Working tree is dirty. Commit or stash first."
