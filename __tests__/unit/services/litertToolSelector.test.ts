...
import { selectRelevantToolsWithModel } from '@offgrid/models';
...
describe('selectRelevantTools', () => {
  ...
  it('returns the single tool named in the reply', async () => {
    ...
    expect(await selectRelevantToolsWithModel('find my notes', tools, mockGenerateToolSelection)).toEqual(['notion-search']);
  });
  ...
  it('matches multiple names regardless of formatting/casing', async () => {
    ...
    expect(await selectRelevantToolsWithModel('weather then add', tools, mockGenerateToolSelection)).toEqual(['web_search', 'calculator']);
  });

  it('returns [] (no tool needed) when the reply is "none"', async () => {
    mockGenerateToolSelection.mockResolvedValueOnce('none');
    expect(await selectRelevantToolsWithModel('hello', tools, mockGenerateToolSelection)).toEqual([]);
  });
  ...
  it('returns null (fall back to all) when the reply names no tool and is not "none"', async () => {
    ...
    expect(await selectRelevantToolsWithModel('do something', tools, mockGenerateToolSelection)).toBeNull();
  });

  it('returns null for empty tools or blank query without calling the model', async () => {
    expect(await selectRelevantToolsWithModel('hi', [], mockGenerateToolSelection)).toBeNull();
    expect(await selectRelevantToolsWithModel('   ', tools, mockGenerateToolSelection)).toBeNull();
  ...
  it('sends a name:description catalog and truncates long descriptions', async () => {
    ...
    await selectRelevantToolsWithModel('q', longTool, mockGenerateToolSelection);
