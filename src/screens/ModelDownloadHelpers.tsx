...
const ServerCard: React.FC<{
  ...
  const serverType = server.endpoint.includes(':11434')
    ? 'Ollama'
    : server.endpoint.includes(':1234')
    ? 'LM Studio'
    : 'AI Server';
      ...
      <View style={styles.serverCardContent}>
        ...
        {!isConnecting && isConnected && (
          <View
            ...
            testID={`discovered-server-${server.id}-connected`}
          >
        ...
        {!isConnecting && !isConnected && (
          <TouchableOpacity
            ...
            onPress={onConnect}
            testID={`discovered-server-${server.id}-connect`}
          >
      ...
      {!isCheckingNetwork && !hasServers && (
        <>
          <Text style={styles.emptyText}>
            No servers found. Make sure you're on the same WiFi network as your
            Off Grid AI Desktop, Ollama, or LM Studio server, then scan or add
            it manually.
          </Text>
          <TouchableOpacity
            onPress={() =>
          ...
          >
            <Text style={[styles.getDesktopLink, { color: colors.primary }]}>
