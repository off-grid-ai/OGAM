...
describe('T117 (rendered) — OS memory warning reclaims idle sidecars (In Memory UI)', () => {
  it('drops the idle whisper sidecar on a memory warning while the text model stays', async () => {
    ...
    await h.rtl.waitFor(() => { expect(before.queryByTestId('resident-item-whisper')).not.toBeNull(); }, { timeout: 4000 });
    expect(before.queryByTestId('resident-item-text')).not.toBeNull();
    before.unmount();
    ...
    await h.rtl.waitFor(() => { expect(after.queryByTestId('resident-item-whisper')).toBeNull(); }, { timeout: 4000 });
