...
export interface LiteRTTurn {
  ...
  content?: string;
  /** Native benchmark counters emitted with completion. */
  benchmark?: { prefillTokenCount?: number; decodeTokenCount?: number };
}
...
function makeLiteRTFake(handle: FakeEmitterHandle): LiteRTFake {
  ...
  const emitCompletion = (turn: LiteRTTurn, includeReasoning = true) => {
    ...
    handle.emit('litert_complete', JSON.stringify(turn.benchmark ?? {}));
  };
  ...
  const module: Record<string, jest.Mock> = {
    ...
    respondToToolCall: jest.fn(() => {
      ...
      if (currentTurn && --toolCallsRemaining <= 0) {
        ...
        defer(() => emitCompletion(turn, false));
      }
      return Promise.resolve();
    }),
...
export interface LlamaFake {
  ...
  /** Give one model path the template its native GGUF metadata would expose. Persistent across loads;
   *  used by model-switch journeys to prove capabilities follow the newly loaded context. */
  setModelChatTemplate(modelPath: string, chatTemplate: string): void;
  /** Set the GGUF-trained context metadata exposed after native load. */
  setModelContextLength(contextLength: number, architecture?: string): void;
...
function makeLlamaFake(
  ...
  const modelChatTemplates = new Map<string, string>();
      ...
      async (
        ...
        const mtpRequested =
          ...
          (!!speculative &&
            ...
            (speculative as { type?: string }).type === 'draft-mtp');
        const mtpEnabled = mtpInitialized && mtpRequested;
        if (mtpEnabled && mtpFailure) {
          const message = mtpFailure;
          mtpFailure = null;
          throw new Error(message);
        }
        const mtpResult = mtpEnabled
          ? {
              draft_tokens: pending.completionMeta?.draft_tokens ?? 6,
              draft_tokens_accepted:
                pending.completionMeta?.draft_tokens_accepted ?? 4,
            }
          : { draft_tokens: 0, draft_tokens_accepted: 0 };
  ...
  const module: Record<string, jest.Mock> = {
    loadLlamaModelInfo: jest.fn(async () =>
      mtpLayers > 0 ? { 'test.nextn_predict_layers': mtpLayers } : {},
    ),
    ...
    initLlama: jest.fn(async (params?: Record<string, unknown>) => {
      const modelPath = String(params?.model ?? '');
      const modelTemplate = modelChatTemplates.get(modelPath);
      if (modelTemplate) {
        const model = (context as Record<string, unknown>).model as {
          metadata: Record<string, string>;
        };
        model.metadata['tokenizer.chat_template'] = modelTemplate;
      }
      ...
      (context as Record<string, unknown>).reasonNoGPU = nativeGpu
        ? ''
        : n > 0
        ? 'native backend refused offload'
        : 'gpu layers not requested';
      return context;
    }),
  ...
  return {
    ...
    scriptCompletion: r => {
      scriptedQueue = [];
      pending = normalizeScript(r);
    },
    scriptCompletions: results => {
      scriptedQueue = results.map(normalizeScript);
      pending = { text: '' };
    },
    releaseStream: () => {
      const f = releaseFn;
    ...
    scriptGpuNativeRefusal: (refuse = true) => {
      gpuNativeRefuses = refuse;
    },
    scriptInitFailure: (fail = true) => {
      initFails = fail;
    },
    scriptMtpFailure: (message = 'MTP speculative decoder unavailable') => {
      mtpFailure = message;
    },
    setModelChatTemplate: (modelPath, modelTemplate) => {
      modelChatTemplates.set(modelPath, modelTemplate);
    },
    setModelContextLength: (contextLength, architecture = 'qwen35') => {
      const model = (context as Record<string, unknown>).model as {
        metadata: Record<string, string>;
      };
      model.metadata['general.architecture'] = architecture;
      model.metadata[`${architecture}.context_length`] = String(contextLength);
    },
...
function makeDiffusionFake(
  ...
  const module: Record<string, jest.Mock> = {
    ...
    loadModel: jest.fn(async (request: string | { modelPath: string }) => {
      loadedModelPath =
        typeof request === 'string' ? request : request.modelPath;
      return true;
    }),
    ...
    deleteGeneratedImage: jest.fn(async (imageId: string) => {
      const imagePath = generatedPaths.get(imageId);
      if (imagePath) {
        await deleteFile?.(imagePath);
        generatedPaths.delete(imageId);
      }
      return true;
    }),
    ...
    generateImage: jest.fn((nativeParams: Record<string, unknown>) => {
      ...
      generatedPaths.set(`img-${seedCounter}`, imagePath);
...
function makeFsFake(): FsFake {
  ...
  vol.writeFileSync('/mock/image.jpg', Buffer.alloc(1024));
  vol.writeFileSync('/mock/camera.jpg', Buffer.alloc(1024));
  ...
  const seedDir = (path: string) =>
    vol.mkdirSync(norm(path), { recursive: true });
  ...
  const module: Record<string, unknown> = {
    ...
    mkdir: jest.fn(async (p: string) => {
      vol.mkdirSync(norm(p), { recursive: true });
    }),
    ...
    stat: jest.fn(async (p: string) =>
      mkStat(p, vol.statSync(norm(p)) as never),
    ),
    writeFile: jest.fn(async (p: string, contents: string) => {
      const np = norm(p);
      vol.mkdirSync(np.slice(0, np.lastIndexOf('/')) || '/', {
        recursive: true,
      });
    ...
    unlink: jest.fn(async (p: string) => {
      vol.rmSync(norm(p), { recursive: true, force: true });
    }),
    moveFile: jest.fn(async (from: string, to: string) => {
      vol.renameSync(norm(from), norm(to));
    }),
    copyFile: jest.fn(async (from: string, to: string) => {
      vol.copyFileSync(norm(from), norm(to));
    }),
...
export function installNativeBoundary(opts: InstallOpts = {}): NativeBoundary {
  ...
  const diffusion = makeDiffusionFake(
    ...
    fsFake
      ? async path => {
          await (fsFake.module.unlink as (p: string) => Promise<void>)(path);
        }
      : undefined,
  );
